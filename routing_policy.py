...
async def build_model_metadata() -> dict:
    metadata = {}
    for entry in MODELS.values():
        model_id = entry["model"]
        quality_info = await get_live_quality_score(model_id, fallback_score=entry["quality_tier"])
        metadata[model_id] = {
...
async def select_model(prompt: str, routing_mode: str = "balanced",
    ...
    tier = classify(prompt)  # rule-based — synchronous, no await needed
    ...
    if not candidates:
        ...
        return {"model": fallback, "tier": tier,
    ...
    return {"model": best["model"], "tier": tier, "reason": reason,
